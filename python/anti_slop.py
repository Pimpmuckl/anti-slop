...
class Bindings(ast.NodeVisitor):
    ...
    def add(self, name: str, node: ast.AST, qualified: str | None = None,
    ...
    def function_header(self, node: ast.FunctionDef | ast.AsyncFunctionDef | ast.Lambda) -> None:
...
class Checker(ast.NodeVisitor):
    def __init__(self, source: str, path: str, tree: ast.Module,
                 selected: frozenset[str], internal_modules: tuple[str, ...]) -> None:
        self.source = source
        ...
        self.locations: dict[ast.stmt, tuple[int, int]] = {}
    ...
    def make_scope(self, node: ast.AST, parent: Scope | None,
                   body: Sequence[ast.AST]) -> Scope:
        bindings = Bindings()
    ...
    def read_ignore(self, token: tokenize.TokenInfo) -> None:
        if not re.match(r"#\s*anti-slop:", token.string):
            return
        match = re.fullmatch(r"#\s*anti-slop:\s*ignore\[([A-Z0-9, \t]+)\]\s*--\s*(\S.*)", token.string)
        codes = {part.strip() for part in match[1].split(",")} if match else set()
        if not match or not codes or not codes <= RULES.keys():
            self.raw(token.start[0], token.start[1] + 1, "ASPY900",
    ...
    def anchors(self, node: ast.expr | ast.stmt) -> set[int]:
        ...
        return {node.lineno, statement.lineno if isinstance(statement, ast.stmt) else node.lineno}

    def emit(self, node: ast.expr | ast.stmt, code: str, message: str) -> None:
        if code not in self.selected:
            return
        matching = [ignore for ignore in self.ignores
    ...
    def precedes(self, definition: ast.AST, use: ast.AST) -> bool:
        ...
        return left is not None and right is not None and left[0] == right[0] and left[1] < right[1]

    def qualified(self, node: ast.AST, scope: Scope | None = None,
                  seen: frozenset[tuple[int, str]] = frozenset()) -> str | None:
        ...
        while current is not None:
            if current.bindings.star_import or node.id in current.bindings.modified_roots:
                return None
    ...
    def local_assignment(self, node: ast.AST, use: ast.AST) -> AssignedValue | None:
        ...
        if (isinstance(assignment, (ast.Assign, ast.AnnAssign))
                and assignment.value is not None and self.precedes(assignment, use)):
            if isinstance(assignment, ast.Assign) and len(assignment.targets) != 1:
                return None
            return AssignedValue(assignment, assignment.value,
                                 assignment.annotation if isinstance(assignment, ast.AnnAssign) else None)
        return None
    ...
    def known_annotation(self, node: ast.AST, use: ast.AST) -> ast.expr | None:
        ...
        return annotation if annotation is not None and not self.broad(annotation) else None
    ...
    def cast_arguments(self, node: ast.AST) -> tuple[ast.expr, ast.expr] | None:
        ...
        for keyword in node.keywords:
            if keyword.arg is None or keyword.arg not in {"typ", "val"} or keyword.arg in values:
                return None
            values[keyword.arg] = keyword.value
        return (values["typ"], values["val"]) if values.keys() == {"typ", "val"} else None

    def json_boundary(self, node: ast.AST, use: ast.AST,
        ...
        if isinstance(node, ast.Name) and node.id not in seen:
            ...
            if assignment is not None:
                return self.json_boundary(assignment.value, assignment.node, seen | {node.id})
        return False
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if arguments is not None:
            ...
            if not self.broad(target) and self.json_boundary(value, node):
                self.emit(node, "ASPY001", "typing.cast does not validate JSON. Parse the boundary value before claiming this type.")
            inner = self.cast_arguments(value)
            if inner is not None and self.broad(inner[0]) and not self.broad(target):
                self.emit(node, "ASPY002", "This nested cast erases type evidence before asserting a narrower type. Keep the original contract.")
            else:
                assignment = self.local_assignment(value, node)
                if assignment is not None:
                    broad = assignment.annotation is not None and self.broad(assignment.annotation)
                    ...
                    if broad and annotation is not None and not self.broad(target):
                        self.emit(node, "ASPY002", f"This binding discarded {ast.unparse(annotation)} and recreates type evidence with a cast. Preserve the original type.")
            if not self.justified(node):
                self.emit(node, "ASPY003", "Explain the invariant in a nearby nonempty # SAFETY: comment. A cast does not check it.")
        ...
        if module and any(module == prefix or module.startswith(prefix + ".") for prefix in self.internal_modules):
            self.emit(node, "ASPY101", "Replace patching application-owned dependencies with an explicit seam or faithful fake. Environment fixtures are not banned.")
        self.generic_visit(node)
    ...
    def visit_Assign(self, node: ast.Assign) -> None:
        if len(node.targets) == 1 and isinstance(node.targets[0], ast.Name):
            ...
            value = node.value
            list_copy = (isinstance(value, ast.BinOp) and isinstance(value.op, ast.Add)
            ...
            while loop is not None and not isinstance(loop, (ast.For, ast.AsyncFor, ast.While)):
            ...
            if loop is not None and len(definitions) == 2 and (list_copy or dict_copy):
                for definition in definitions:
                    ...
                    if isinstance(initial, (ast.Assign, ast.AnnAssign)) and self.precedes(initial, loop):
                        literal = ast.List if list_copy else ast.Dict
                        if isinstance(initial.value, literal):
                            self.emit(node, "ASPY004", "This loop copies its growing local accumulator. Build it directly when ownership permits; mutation is not an automatic safe rewrite.")
        self.generic_visit(node)
    ...
    def finish(self) -> list[Diagnostic]:
        for ignore in self.ignores:
            ...
            if unused:
                self.raw(ignore.line, ignore.column, "ASPY901", "Unused suppression: " + ", ".join(sorted(unused)))
        return sorted(self.diagnostics, key=lambda item: (item.path, item.line, item.column, item.code))


def check_source(source: str, path: str = "<string>", *,
                 selected: frozenset[str] = DEFAULT_RULES,
                 internal_modules: tuple[str, ...] = ()) -> list[Diagnostic]:
    """Check source without importing or executing it; positions are one-based."""
    try:
        tree = ast.parse(source, filename=path)
    except SyntaxError as error:
        return [Diagnostic(path, error.lineno or 1, error.offset or 1, "ASPY000", error.msg)]
...
def configuration(explicit: str | None) -> tuple[dict[str, object], Path]:
    candidates = [Path(explicit)] if explicit else [parent / "pyproject.toml" for parent in [Path.cwd(), *Path.cwd().parents]]
    for path in candidates:
        if explicit or path.is_file():
            with path.open("rb") as file:
                document = tomllib.load(file)
            settings = table(table(table(document, "tool"), "anti-slop"), "python")
            unknown = settings.keys() - {"select", "extend-select", "ignore", "exclude", "internal-modules"}
            if unknown:
                raise ValueError("Unknown Python anti-slop settings: " + ", ".join(sorted(unknown)))
        ...
        if path.is_file():
            ...
            for directory, dirs, files in os.walk(path, onerror=fail, followlinks=False):
                ...
                for name in files:
                    ...
                    if file.suffix in {".py", ".pyi"} and not file.is_symlink() and not excluded(file):
                        found.add(file)
        else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--config", help="pyproject.toml containing [tool.anti-slop.python]")
    parser.add_argument("--format", choices=("text", "json"), default="text")
    parser.add_argument("--select", help="comma-separated exact rule codes; replaces the selection")
    ...
    try:
        ...
        selected = set(string_list(settings, "select")) if "select" in settings else set(DEFAULT_RULES)
        ...
        if any(not re.fullmatch(r"[A-Za-z_]\w*(?:\.[A-Za-z_]\w*)*", module) for module in modules):
            raise ValueError("internal-modules must contain dotted Python module names")
        if "ASPY101" in selected and not modules:
            raise ValueError("ASPY101 requires explicit internal-modules; no application ownership is guessed")
        files = discover(args.paths or ["."], root, string_list(settings, "exclude"))
        diagnostics = []
        for file in files:
            with tokenize.open(file) as stream:
                source = stream.read()
            diagnostics.extend(check_source(source, str(file), selected=frozenset(selected), internal_modules=modules))
    ...
    return 2 if any(item.code == "ASPY000" for item in diagnostics) else int(bool(diagnostics))
