...
Put the comment on the cast/containing statement line, or an immediately preceding
comment block. Blank lines or another statement break attachment. A comment does
not silence ASPY001/002. Never manufacture meaningless justifications as fixes.

An intentional exception needs exact rule codes and a reason, on the statement or
the immediately preceding line:

```python
user = cast(User, sdk_result)  # anti-slop: ignore[ASPY003] -- contract documented in the SDK issue
```
