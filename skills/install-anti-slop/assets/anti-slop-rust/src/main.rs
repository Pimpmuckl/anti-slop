...
fn main() -> ExitCode {
    ...
    if args
        .first()
        .is_some_and(|arg| arg == OsStr::new("anti-slop"))
    {
        args.remove(0);
    }
    if args.len() == 1 {
    ...
    match Command::new(cargo).args(clippy_args(&args)).status() {
        ...
        Err(error) => {
            eprintln!(
                "anti-slop: could not run Cargo: {error}. Install Rust and the Clippy component."
            );
            ExitCode::from(2)
        }
    }
...
mod tests {
    ...
    fn preserves_cargo_options_and_trailing_rustc_overrides() {
        ...
        assert_eq!(
            &args[..4],
            &arguments(&["clippy", "--manifest-path", "a path/Cargo.toml", "--lib"])
        );
